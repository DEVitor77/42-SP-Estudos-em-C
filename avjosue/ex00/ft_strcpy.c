/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_strcpy.c                                        :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: vvitor-d <vvitor-d@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/18 11:28:35 by josupere          #+#    #+#             */
/*   Updated: 2023/03/21 17:51:50 by vvitor-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <stdio.h>

char	*ft_strcpy(char *dest, char *src)
{
	int	c;

	c = 0;
	while (src[c] != '\0')
	{
		dest[c] = src[c];
		c++;
	}
	dest[c] = '\0';
	return (dest);
}
#include <stdio.h>
#include <strings.h>

char	*ft_strcpy(char *dest, char *src);

void	set_str(char *src, char *dest)
{
	int	pos;

	pos = 0;
	while (src[pos] != '\0')
	{
		dest[pos] = src[pos];
		pos++;
	}
	dest[pos] = '\0';
}

void	reset_strs(char *a, char *b, char *c, char *d)
{
	set_str("aaaaaaaaaaaaaaaaaaa", a);
	set_str("aaaaaaaaaaaaaaaaaaa", b);
	set_str("bbbbbbbbbbbbbbbbbbb", c);
	set_str("bbbbbbbbbbbbbbbbbbb", d);
}

int	same_result(char *expected, char *output, int len)
{
	int		pos;
	int		res;
	char	e;
	char	o;

	res = 1;
	pos = 0;
	while (pos < len)
	{
		e = expected[pos];
		o = output[pos];
		if (e != o)
		{
			res = 0;
		}
		pos++;
	}
	if (!res)
		printf("'%s' : '%s' \n", expected, output);
	return (res);
}

int	main(void)
{
	char	src[20];
	char	dest[20];
	char	std_src[20];
	char	std_dest[20];

	reset_strs(src, std_src, dest, std_dest);
	set_str("", src);
	set_str("", std_src);
	set_str("efgh", dest);
	set_str("efgh", std_dest);
	printf("%d\n", same_result(strcpy(std_dest, std_src), ft_strcpy(dest, src),
				3));
	reset_strs(src, std_src, dest, std_dest);
	set_str("", dest);
	set_str("", std_dest);
	printf("%i\n", same_result(strcpy(std_dest, "abc"), ft_strcpy(dest, "abc"),
				2));
	reset_strs(src, std_src, dest, std_dest);
	set_str("ab", src);
	set_str("ab", std_src);
	set_str("efghij005klm", dest);
	set_str("efghijklm", std_dest);
	printf("%d\n", same_result(strcpy(std_dest, std_src), ft_strcpy(dest, src),
				3));
}
